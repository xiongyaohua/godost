...
private:
    ...
    Ref<Image> placeholder; // A placeholder image shown when there is no actual typst pages.
...
public:
	...
	void set_placeholder(const Ref<Image> &p_image) { placeholder = p_image; }
	Ref<Image> get_placeholder() { return placeholder; }
	...
	Ref<Image> get_page(const size_t p_page);
	size_t get_page_count() { return pages.size(); }
	void show_page(size_t p_page);

	bool is_ready();
	String invoke_compilation(bool p_is_async = false);
	void cancel_compilation();
};
